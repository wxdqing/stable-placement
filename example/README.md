...
目录 `example/bdd/` 提供面向真实 Redis 的 BDD 风格验收测试，默认连接 `127.0.0.1:6379`。

### 前置条件

- Redis 已监听 `6379`（本地或开发机）
- BDD 密码只通过 `STABLE_PLACEMENT_REDIS_PASSWORD` 环境变量注入
...
| Node 续约 | RenewNode 推进 NodeLeaseVersion，并使用持久化 TTLMillis |
| 释放后禁止 Recover | Release → Recover 返回 `ErrPlacementNotRecoverable` |
| Node Lease 到期后可 Recover | Placement 逻辑失效但记录保留，再显式 Recover |
...
真实 Redis 专项测试使用独立地址和密码变量：

```bash
STABLE_PLACEMENT_REAL_REDIS_ADDR=127.0.0.1:6379 \
  STABLE_PLACEMENT_REAL_REDIS_PASSWORD="$STABLE_PLACEMENT_REAL_REDIS_PASSWORD" \
  go test -tags=integration ./redis/... -run RealRedis -v
```

认证密码只从 `STABLE_PLACEMENT_REAL_REDIS_PASSWORD` 环境变量读取，不写入命令、文档或仓库配置。

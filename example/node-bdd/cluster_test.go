...
func TestCluster_A3_RenewNodeSessionValidation(t *testing.T) {
	...
	defer ticker.Stop()
	for {
		_, err := h.dir.RenewNode(h.ctx, sp.RenewNodeCommand{NodeIdentity: node.NodeIdentity, NodeSessionID: node.NodeSessionID})
	...
	_, err := h.dir.RenewNode(h.ctx, sp.RenewNodeCommand{NodeIdentity: node.NodeIdentity, NodeSessionID: "wrong-session"})

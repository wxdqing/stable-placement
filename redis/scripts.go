...
const resolveRouteLua = luaHelpers + `
...
if existing and existing["Status"]=="active" then
  ...
  if owner_usable and owner["NodeSessionID"]~=existing["OwnerNodeSessionID"] then
    ...
    local encoded=cjson.encode(existing);redis.call("SET",KEYS[1],encoded);event(KEYS[7],ARGV[8],ARGV[3],owner["NodeIdentity"],owner["NodeSessionID"],tostring(existing["Version"]),tostring(owner["Lease"]["Version"]))
...
if recovering then
  ...
  p={GrainID=ARGV[1],Kind=ARGV[2],GrainKey=ARGV[3],NodeIdentity=chosen.node["NodeIdentity"],OwnerNodeSessionID=chosen.node["NodeSessionID"],Version=version,Status="active",CreateTimeUnixMilli=now,UpdateTimeUnixMilli=now}
end
local encoded=cjson.encode(p)
...
event(KEYS[7],event_type,ARGV[3],chosen.node["NodeIdentity"],chosen.node["NodeSessionID"],tostring(version),tostring(chosen.node["Lease"]["Version"]))

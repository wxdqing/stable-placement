...
if p["GrainKey"]~=ARGV[8] then return "placement_not_found" end;if p["Status"]~="active" then if ARGV[1]=="recover" then return "not_recoverable" end;return "placement_not_found" end;if tostring(p["Version"])~=ARGV[6] then return "version_conflict" end
